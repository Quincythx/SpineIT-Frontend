...
export function ReviewSnippetCard({ review }: { review: Review }) {
  return (
    <Link
      ...
      className="bg-white border-l-4 border-accent drop-shadow-[0px_4px_10px_rgba(0,0,0,0.05)] rounded-lg pl-7 pr-6 py-6 flex flex-col gap-4 flex-1 min-w-0"
    >
      <div className="flex items-center gap-3">
        <Avatar name={review.user} size={40} />
        <div>
          <p className="font-['Inter'] font-semibold text-sm text-ink">{review.user}</p>
          <p className="font-['Inter'] font-medium text-xs text-ink-muted">
            Reviewed <span className="italic">{review.book.title}</span>
          </p>
        </div>
      </div>
      <p className="font-['Inter'] text-base text-ink leading-6 line-clamp-3">{review.review_text}</p>
